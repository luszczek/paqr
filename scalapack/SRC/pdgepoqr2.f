      SUBROUTINE PDGEPOQR2( M, N, A, IA, JA, DESCA, TAU, ND, DEFF,
     $                      WORK, LWORK, INFO )
*
...
*
*     .. Scalar Arguments ..
      INTEGER            IA, INFO, JA, LWORK, M, N, ND
*     ..
*     .. Array Arguments ..
      INTEGER            DESCA( * ), DEFF( * )
...
*     .. External Subroutines ..
     ...
     $                   PDELSET, PDLARF, PDLARFGPO, PB_TOPGET,
     $                   PB_TOPSET, PXERBLA
*     ..
*     .. External Functions ..
...
*           A(i+1:ia+m-1,j)
*
!            CALL PDLARFG( M-J+JA+ND, AJJ, I, J, A, MIN( I+1, IA+M-1 ),
!     $                      J, DESCA, 1, TAU )
            CALL PDLARFGPO( M-J+JA+ND, AJJ, I, J, A, MIN( I+1, IA+M-1 ),
     $                      J, DESCA, 1, TAU , ND, DEFF(J) )
!      write(*,*) '&', ND, DEFF(1), DEFF(2)
!      CALL BLACS_ALLREDUCE( ICTXT, ND )
!      CALL BLACS_ALLREDUCE( ICTXT, DEFF(J) )
!      write(*,*) '$', J, JA, JA+K-1, DEFF(J)
            IF( DEFF(J).EQ.1 ) THEN
                ND = ND + 1
            ELSE
                IF( J.LT.JA+N-1 ) THEN
...
*    
                   CALL PDELSET( A, I, J-ND, DESCA, ONE )
*    
                   CALL PDLARF( 'Left', M-J+JA+ND, N-J+JA-1, A, I, J-ND,
     $                          DESCA, 1, TAU, A, I, J+1, DESCA, WORK )
                END IF
