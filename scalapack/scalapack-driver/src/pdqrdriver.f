...
*     .. Local Scalars ..
      ...
      CHARACTER       filename_A*4
      CHARACTER       filename_R*4
      INTEGER            II, JJ
      LOGICAL            CHECK
      INTEGER            I, IAM, IASEED, ICTXT, IMIDPAD, INFO, IPA,
     $                   IPOSTPAD, IPPIV, IPREPAD, IPTAU, IPW, IPDEFF,
     $                   NRHS, J, K, KFAIL, KPASS, KSKIP, KTESTS, L, LIPIV, LTAU,
...
*     .. External Subroutines ..
     ...
     $                   PDQPPIV, PDGEQRF, PDGEPOQRF, PDGEQRRV,
...
*     
                     ...
                     WORKFCT = LWORK + IPOSTPAD
                     WORKSIZ = WORKFCT
                     IPDEFF = IPW + LWORK + IPOSTPAD + IPREPAD 
                     NRHS = 0
*     
                     IF( CHECK ) THEN
*     
...
!       write (filename_A, '(I4)') IAM
!       open (unit=20, file=trim(FILEDIR)//'A'//filename_A, STATUS='OLD')
!       read(20, *) MEM( IPA : IPA + 3)
!       close(20)
!       write(*, *) IAM, '~', MEM( IPA + 0), MEM( IPA + 1),
!     $               MEM( IPA + 2), MEM( IPA + 3)
...
*     
     ...
     $                             INFO )
                  ...
                  ELSE IF( LSAMEN( 2, FACT, 'PO' ) ) THEN
                     CALL SLTIMER( 1 )
                     CALL PDGEPOQRF( M, N-NRHS, NRHS, MEM( IPA ), 1, 1,
     $                    DESCA, MEM( IPTAU ), ND, MEM( IPDEFF ),
     $                    MEM( IPW ), LWORK, INFO )
                     CALL SLTIMER( 1 )
                  ELSE IF( LSAMEN( 2, FACT, 'QL' ) ) THEN
                     CALL SLTIMER( 1 )
     ...
     $                                INFO )
                  ...
                  END IF
*     
!       write (filename_R, '(I4)') IAM
!       open (unit=30, file=trim(FILEDIR)//'R'//filename_R,
!     $ STATUS='UNKNOWN')
!       write(30, *) MEM( IPA : IPA + 3)
!       close(30)
!        MEM( IPA + 4) = 1
!       write(*, *) IAM, '!', MEM( IPA + 0), MEM( IPA + 1),
!     $             MEM( IPA + 2), MEM( IPA + 3)
