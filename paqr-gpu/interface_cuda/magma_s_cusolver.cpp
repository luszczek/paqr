/*
    -- MAGMA (version 2.0) --
       ...
       @generated from interface_cuda/magma_z_cusolver.cpp, normal z -> s, Tue Apr 12 21:49:48 2022
*/
...
#define magma_sgetrf_cusolver               cusolverDnZgetrf
#define magma_sgetrf_cusolver_bufferSize    cusolverDnZgetrf_bufferSize

#define magma_sgeqrf_cusolver               cusolverDnZgeqrf
#define magma_sgeqrf_cusolver_bufferSize    cusolverDnZgeqrf_bufferSize

#define magma_spotrf_cusolver               cusolverDnZpotrf
#define magma_spotrf_cusolver_bufferSize    cusolverDnZpotrf_bufferSize

#define magma_spotrf_batched_cusolver_w     cusolverDnZpotrfBatched
    ...
    magma_int_t *info )
{
    ...
    magma_sgetrf_cusolver_bufferSize(handle, (int)m, (int)n, (float*)dA, (int)ldda, &lwork);
    if( MAGMA_SUCCESS != magma_smalloc(        &dwork, (magma_int_t)lwork  ) ||
    ...
    magma_sgetrf_cusolver( handle,
                           (int)m, (int)n,
                           (float*)dA,    (int)ldda,
                           (float*)dwork, (int*)dipiv,
                           (int*)dinfo );
    ...
    magma_int_t *info )
{
    ...
    magma_int_t min_mn = min(m, n);
    magmaFloat_ptr dwork=NULL, dtau=NULL;
    magma_queue_t queue;
    magma_device_t cdev;
    magma_getdevice( &cdev );
    magma_queue_create( cdev, &queue );
    ...
    magma_sgeqrf_cusolver_bufferSize(handle, (int)m, (int)n, (float*)dA, (int)ldda, &lwork);
    if( ( MAGMA_SUCCESS != magma_smalloc( &dwork, (magma_int_t)lwork ) ) ||
        ( MAGMA_SUCCESS != magma_smalloc( &dtau, min_mn )              ) ||
        ( MAGMA_SUCCESS != magma_malloc((void**)&dinfo, 1*sizeof(int)) )
      ){
          (*info) = MAGMA_ERR_DEVICE_ALLOC;
          goto cleanup;
    ...
    magma_sgetvector(min_mn, dtau, 1, tau, 1, queue);
    ...
    magma_int_t *info )
{
    ...
    magma_spotrf_cusolver_bufferSize(handle, cusolver_uplo, (int)n, (float*)dA, (int)ldda, &lwork);
    ...
    magma_spotrf_cusolver( handle, cusolver_uplo, (int)n,
                           (float*)dA, (int)ldda, dwork, (int)lwork, (int*)dinfo );
    ...
    magma_queue_t queue)
{
    ...
    magma_spotrf_batched_cusolver_w(handle, cusolver_uplo, n, dA_array, ldda, info_array, batchCount);
    cusolverDnDestroy(handle);
    magma_queue_sync( queue );
    return 0;
}
