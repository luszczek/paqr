/*
    -- MAGMA (version 2.0) --
       ...
       @generated from testing/testing_zpaqrf_batched.cpp, normal z -> c, Tue Apr 12 21:49:53 2022
...
//    row2, col1, matrix2
//    ...
int genmat(int radius, int batch, int* outrow, int* outcol, float** outmat)
{
    ...
    *outcol = Ncol;
    ...
    float* mat = (float*) malloc(Nmat * Nrow * Ncol * sizeof(float));
    *outmat = mat;
    ...
    float* mom = (float*) malloc(Ntot * Ncol * sizeof(float));
    ...
    float** mvals = (float**) malloc(Ntot * sizeof(float*));
    for (int ip=0; ip < Ntot; ip++)
    {
      ...
      for (int d=0; d < 3; d++)
      {
        xc[d] = (float) rand() / (float) RAND_MAX;
    ...
    float** avals = (float**) malloc(Nmat * sizeof(float*));
    for (int ip=0; ip < Nmat; ip++)
    {
      ...
      for (int r=0; r < Nrow; r++)
      {
        ...
        float dist = sqrt(dx0[0]*dx0[0]+dx0[1]*dx0[1]+dx0[2]*dx0[2]);
        float weight = pow(dist,-P-1);
        ...
        for (int ic=0; ic < Ncol; ic++)
        {
          ...
          for (int is=0; is < Ncol; is++) // shifted location
          {
            ...
            */
            float shift = pCq*pow(dx0[0],p[0]-q[0])
                              *pow(dx0[1],p[1]-q[1])
                              *pow(dx0[2],p[2]-q[2]);
            // Fix for centroid entries which should be 0
            float momq = ((q[0]+q[1]+q[2])==1) ? 0 : mvals[ixr][is];
...
int main( int argc, char** argv)
{
    ...
    magmaFloatComplex *h_A, *h_R, *h_Amagma, *h_R2, *tau, *h_work, tmp[1], unused[1];
    ...
    float tol = 30. * lapackf77_slamch("E");
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            // in real, the genmat function allocates & initializes the space for h_A
            #ifdef PRECISION_d
            if(opts.version  == 3) {
            ...
            TESTING_CHECK( magma_cmalloc_cpu( &h_R2, n2 ));
            ...
            for(magma_int_t s = 0; s < N*batchCount; s++) {
                ...
                magmaFloatComplex *hcol = h_A + s * lda;
                float colnorm   = lapackf77_clange( "F", &M, &ione, hcol, &lda, (float*)h_work );
                if( colnorm == MAGMA_D_ZERO ) colnorm = MAGMA_D_ONE;

                beta = MAGMA_C_MAKE(1./colnorm, MAGMA_D_ZERO);
                blasf77_cscal( &M, &beta, hcol, &ione );
            }
            ...
            if(opts.nrhs > 1) {
                for(magma_int_t s = 0; s < batchCount; s++) {
                    magmaFloatComplex *hTmp = h_A + s * lda * N;
                    for(magma_int_t icol = 0; icol < N; icol++) {
                        if(icol % opts.nrhs == 0) {
                            magmaFloatComplex *hcol = hTmp + icol * lda;
                            memset(hcol, 0, M*sizeof(magmaFloatComplex));
                        }
                    }
                }
            }
            ...
            if(opts.version == 1 || opts.version == 3) {
                ...
                float paqr_tol = lapackf77_slamch("E") * opts.tolerance;
                info = magma_cpaqr2_fused_sm_batched(
                        M, N,
                        dA_array, 0, 0, ldda,
                        dtau_array, 0,
                        dflag_array, 0,
            ...
            if (opts.check) {
                ...
                for (int i=0; i < batchCount; i++) {
                    ...
                    magmaFloatComplex *hR1 = h_R  + i*lda*N;
                    magmaFloatComplex *hR2 = h_R2 + i*lda*N;
                    magma_int_t *iflag = hflag + i * min_mn;
                    magma_int_t new_N  = 0;
                    for(magma_int_t ic = 0; ic < N; ic++) {
                        if(iflag[ic] == 0) {
                            memcpy(hR2 + new_N * lda, hR1 + ic * lda, lda * sizeof(magmaFloatComplex));
                            new_N++;
                        }
                    }

                    if(batchCount == 1) {
                        magma_cprint(M,     N, hR1, lda);
                        magma_cprint(M, new_N, hR2, lda);
                    }
