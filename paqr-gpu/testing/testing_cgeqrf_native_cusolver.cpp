/*
    -- MAGMA (version 2.0) --
       ...
       @generated from testing/testing_zgeqrf_native_cusolver.cpp, normal z -> c, Tue Apr 12 21:49:53 2022
*/
// includes, system
#include <stdlib.h>
#include <stdio.h>
...
get_QR_error( 
    ...
    magmaFloatComplex_ptr h_A,
    magmaFloatComplex_ptr h_R,
    magma_int_t lda, 
    magmaFloatComplex_ptr d_A, 
    magma_int_t ldda, 
    magmaFloatComplex_ptr tau, 
    magmaFloatComplex_ptr dT, 
    magmaFloatComplex_ptr h_work, 
    magma_int_t lwork, 
    float *error, 
    float *error2)
{
...
void get_QR_error_solve( 
    magma_opts* opts, 
    magma_int_t M, magma_int_t N, 
    magmaFloatComplex_ptr h_A, magma_int_t lda, 
    magmaFloatComplex_ptr d_A, magma_int_t ldda, 
    magmaFloatComplex_ptr tau, 
    magmaFloatComplex_ptr dT, 
    float *error)
{
    ...
    if ( opts->version == 1 ) {
        ...
        if (info != 0) {
            printf("magma_cgeqrs returned magma_error %lld: %s.\n",
        ...
        magma_cgeqrs3_gpu( M, N, 1,
                           ...
                           d_B, M, hwork, lwork2, &info );
        if (info != 0) {
            printf("magma_cgeqrs3 returned magma_error %lld: %s.\n",
                   (long long) info, magma_strerror( info ));
        }
        magma_free_cpu( hwork );
    }
...
int main( int argc, char** argv)
{
    ...
    float           magma_error=0, magma_error2=0, cusolver_error=0, cusolver_error2=0;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if (info != 0) {
                printf("magma_cgeqrf returned magma_error %lld: %s.\n",
                       (long long) info, magma_strerror( info ));
            }
            ...
            if( opts.version == 2){
                ...
                magma_cgeqrf_cusolver_gpu(M, N, d_A, ldda, tau, &info );
                magma_queue_sync( NULL );
                cusolver_time = magma_wtime() - cusolver_time;
                cusolver_perf = gflops / cusolver_time;
                if (info != 0) {
                    printf("magma_cgeqrf_cusolver_gpu returned error %lld: %s.\n",
                           (long long) info, magma_strerror( info ));
                    cusolver_perf = 0.0;
                    cusolver_time = 0.0;
                }
            ...
            if ( opts.lapack ) {
                ...
                if (info != 0) {
                    printf("lapackf77_cgeqrf returned magma_error %lld: %s.\n",
                           (long long) info, magma_strerror( info ));
                }
            }
