/*
    -- MAGMA (version 2.0) --
       ...
       @generated from testing/testing_zpaqrf_batched.cpp, normal z -> d, Tue Apr 12 21:49:53 2022
...
int main( int argc, char** argv)
{
    ...
    double *h_A, *h_R, *h_Amagma, *h_R2, *tau, *h_work, tmp[1], unused[1];
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_CHECK( magma_dmalloc_cpu( &h_R2, n2 ));
            ...
            for(magma_int_t s = 0; s < N*batchCount; s++) {
                ...
                double *hcol = h_A + s * lda;
                double colnorm   = lapackf77_dlange( "F", &M, &ione, hcol, &lda, (double*)h_work );
                if( colnorm == MAGMA_D_ZERO ) colnorm = MAGMA_D_ONE;

                beta = MAGMA_D_MAKE(1./colnorm, MAGMA_D_ZERO);
                blasf77_dscal( &M, &beta, hcol, &ione );
            }
            ...
            if(opts.nrhs > 1) {
                for(magma_int_t s = 0; s < batchCount; s++) {
                    double *hTmp = h_A + s * lda * N;
                    for(magma_int_t icol = 0; icol < N; icol++) {
                        if(icol % opts.nrhs == 0) {
                            double *hcol = hTmp + icol * lda;
                            memset(hcol, 0, M*sizeof(double));
                        }
                    }
                }
            }
            ...
            if(opts.version == 1 || opts.version == 3) {
                ...
                info = magma_dpaqr2_fused_sm_batched(
                        M, N,
                        dA_array, 0, 0, ldda,
                        dtau_array, 0,
                        dflag_array, 0,
            ...
            if (opts.check) {
                ...
                for (int i=0; i < batchCount; i++) {
                    ...
                    double *hR1 = h_R  + i*lda*N;
                    double *hR2 = h_R2 + i*lda*N;
                    magma_int_t *iflag = hflag + i * min_mn;
                    magma_int_t new_N  = 0;
                    for(magma_int_t ic = 0; ic < N; ic++) {
                        if(iflag[ic] == 0) {
                            memcpy(hR2 + new_N * lda, hR1 + ic * lda, lda * sizeof(double));
                            new_N++;
                        }
                    }

                    if(batchCount == 1) {
                        magma_dprint(M,     N, hR1, lda);
                        magma_dprint(M, new_N, hR2, lda);
                    }
