...
get_QR_error( 
    ...
    magmaDoubleComplex_ptr h_A,
    magmaDoubleComplex_ptr h_R,
    magma_int_t lda, 
    magmaDoubleComplex_ptr d_A, 
    magma_int_t ldda, 
    magmaDoubleComplex_ptr tau, 
    magmaDoubleComplex_ptr dT, 
    magmaDoubleComplex_ptr h_work, 
    magma_int_t lwork, 
    double *error, 
    double *error2)
{
...
void get_QR_error_solve( 
    magma_opts* opts, 
    magma_int_t M, magma_int_t N, 
    magmaDoubleComplex_ptr h_A, magma_int_t lda, 
    magmaDoubleComplex_ptr d_A, magma_int_t ldda, 
    magmaDoubleComplex_ptr tau, 
    magmaDoubleComplex_ptr dT, 
    double *error)
{
    ...
    if ( opts->version == 1 ) {
        ...
        if (info != 0) {
            printf("magma_zgeqrs returned magma_error %lld: %s.\n",
        ...
        magma_zgeqrs3_gpu( M, N, 1,
                           ...
                           d_B, M, hwork, lwork2, &info );
        if (info != 0) {
            printf("magma_zgeqrs3 returned magma_error %lld: %s.\n",
                   (long long) info, magma_strerror( info ));
        }
        magma_free_cpu( hwork );
    }
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if (info != 0) {
                printf("magma_zgeqrf returned magma_error %lld: %s.\n",
                       (long long) info, magma_strerror( info ));
            }
            ...
            if( opts.version == 2){
                ...
                magma_zgeqrf_cusolver_gpu(M, N, d_A, ldda, tau, &info );
                magma_queue_sync( NULL );
                cusolver_time = magma_wtime() - cusolver_time;
                cusolver_perf = gflops / cusolver_time;
                if (info != 0) {
                    printf("magma_zgeqrf_cusolver_gpu returned error %lld: %s.\n",
                           (long long) info, magma_strerror( info ));
                    cusolver_perf = 0.0;
                    cusolver_time = 0.0;
                }
            ...
            if ( opts.lapack ) {
                ...
                if (info != 0) {
                    printf("lapackf77_zgeqrf returned magma_error %lld: %s.\n",
                           (long long) info, magma_strerror( info ));
                }
            }
