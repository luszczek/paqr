/*
    -- MAGMA (version 2.0) --
       ...
       @generated from testing/testing_zgeqrf_native_cusolver.cpp, normal z -> s, Tue Apr 12 21:49:53 2022
*/
// includes, system
#include <stdlib.h>
#include <stdio.h>
...
void get_QR_error_solve( 
    magma_opts* opts, 
    magma_int_t M, magma_int_t N, 
    magmaFloat_ptr h_A, magma_int_t lda, 
    magmaFloat_ptr d_A, magma_int_t ldda, 
    magmaFloat_ptr tau, 
    magmaFloat_ptr dT, 
    float *error)
{
    ...
    if ( opts->version == 1 ) {
        ...
        if (info != 0) {
            printf("magma_sgeqrs returned magma_error %lld: %s.\n",
        ...
        magma_sgeqrs3_gpu( M, N, 1,
                           ...
                           d_B, M, hwork, lwork2, &info );
        if (info != 0) {
            printf("magma_sgeqrs3 returned magma_error %lld: %s.\n",
                   (long long) info, magma_strerror( info ));
        }
        magma_free_cpu( hwork );
    }
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if (info != 0) {
                printf("magma_sgeqrf returned magma_error %lld: %s.\n",
                       (long long) info, magma_strerror( info ));
            }
            ...
            if( opts.version == 2){
                ...
                magma_sgeqrf_cusolver_gpu(M, N, d_A, ldda, tau, &info );
                magma_queue_sync( NULL );
                cusolver_time = magma_wtime() - cusolver_time;
                cusolver_perf = gflops / cusolver_time;
                if (info != 0) {
                    printf("magma_sgeqrf_cusolver_gpu returned error %lld: %s.\n",
                           (long long) info, magma_strerror( info ));
                    cusolver_perf = 0.0;
                    cusolver_time = 0.0;
                }
            ...
            if ( opts.lapack ) {
                ...
                if (info != 0) {
                    printf("lapackf77_sgeqrf returned magma_error %lld: %s.\n",
                           (long long) info, magma_strerror( info ));
                }
            }
