/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas/magmablas_zpaqr2_batched_fused_sm.cu, normal z -> c, Tue Apr 12 21:49:49 2022
*/

#include <cuda.h>    // for CUDA_VERSION
#include "magma_internal.h"
#include "magma_templates.h"
#include "magmablas_cpaqr2_batched_fused.cuh"
...
zpaqr2_fused_sm_kernel_batched(
    ...
    magma_int_t** iflag_array, magma_int_t iflag_i,
    magma_int_t *info_array, float tol,
    magma_int_t batchCount)
{
    ...
    magma_int_t* iflag = iflag_array[batchid] + iflag_i;
    ...
    for(int j = 0; j < N; j++){
        ...
        // have the diagonal
        tmp = (tx == ir) ? MAGMA_C_MAKE(beta, MAGMA_D_ZERO) : sA(tx, j);
...
magma_cpaqr2_fused_sm_batched(
    magma_int_t m, magma_int_t n,
    magmaFloatComplex** dA_array, magma_int_t Ai, magma_int_t Aj, magma_int_t ldda,
    magmaFloatComplex **dtau_array, magma_int_t taui,
    magma_int_t** iflag_array, magma_int_t iflag_i,
    magma_int_t* info_array, magma_int_t nthreads, magma_int_t check_launch_only,
    float tol, magma_int_t batchCount, magma_queue_t queue )
{
    ...
    // disable this kernel for n > m
    if( m < n ) return -100;

    //float tol = lapackf77_slamch("E");
    ...
    if (shmem <= shmem_max) {
        cudaFuncSetAttribute(zpaqr2_fused_sm_kernel_batched, cudaFuncAttributeMaxDynamicSharedMemorySize, shmem);
    }
    #else
    ...
    void *kernel_args[] = {&m, &n, &dA_array, &Ai, &Aj, &ldda, &dtau_array, &taui, &iflag_array, &iflag_i, &info_array, &tol, &batchCount};
    cudaError_t e = cudaLaunchKernel((void*)zpaqr2_fused_sm_kernel_batched, grid, threads, kernel_args, shmem, queue->cuda_stream());
    if( e != cudaSuccess ) {
